...
module OoO_top #(
  ...
  // ============================================================================
  // SINGLE SHADOW RESTRICTION LOGIC
  // ============================================================================
  logic [3:0] branches_in_flight;
  logic stall_branch_dispatch;

  always_ff @(posedge clk) begin
      if (rst || branch_mispredict) begin
          branches_in_flight <= 0;
      end else begin
          if (dispatch_alloc_branch && !branch_wb_valid)
              branches_in_flight <= branches_in_flight + 1;
          else if (!dispatch_alloc_branch && branch_wb_valid)
              branches_in_flight <= branches_in_flight - 1;
      end
  end
  assign stall_branch_dispatch = (branches_in_flight > 0);
  ...
  dispatch dispatch_unit (
      ...
      .i_stall_dispatch(stall_branch_dispatch),
  ...
  // ============================================================================
  // CDB ARBITRATION (FIXED)
  // ============================================================================
  always_comb begin
      if (branch_wb_valid) begin
          cdb_valid = 1'b1;
          cdb_tag   = branch_cdb_tag;
          // FIX: Broadcast the actual destination register (e.g., P5 for JAL)
  ...
  rob #(.ROB_WIDTH(4), .PREG_WIDTH(7)) rob_inst (
      ...
      .o_commit_tag(commit_tag), .branch_mispredict(branch_mispredict),
  ...
  reservation_station #(.PREG_WIDTH(7), .ROB_WIDTH(4), .RS_SIZE(8)) rs_alu_inst (
      ...
      .o_full(alu_rs_full), .i_eu_ready(alu_rs_enable), .o_issue_valid(alu_issue_valid), 
      .o_issue_prs1(alu_issue_prs1), .o_issue_prs2(alu_issue_prs2), .o_issue_prd(alu_issue_prd),
      ...
      .branch_mispredict(branch_mispredict), .i_mispredict_tag(branch_cdb_tag),
  ...
  reservation_station #(.PREG_WIDTH(7), .ROB_WIDTH(4), .RS_SIZE(8)) rs_branch_inst (
      ...
      .i_prs1(skid_to_dispatch_prs1), .i_prs2(skid_to_dispatch_prs2), 
      .i_prd(skid_to_dispatch_prd), .i_rob_tag(skid_to_dispatch_rob_tag), 
      .i_imm(skid_to_dispatch_immediate), .i_alu_op(skid_to_dispatch_alu_op), 
      .i_alusrc(1'b0), .i_memwrite(1'b0), .i_cdb_valid(cdb_valid), .i_cdb_prd(cdb_prd),
      .i_rs1_ready((skid_to_dispatch_prs1 == 0) || phys_reg_busy[skid_to_dispatch_prs1]),
      .i_rs2_ready((skid_to_dispatch_prs2 == 0) || phys_reg_busy[skid_to_dispatch_prs2]),
      .o_full(branch_rs_full), .i_eu_ready(1'b1),
      ...
      .o_issue_memwrite(), .branch_mispredict(branch_mispredict),
      .i_mispredict_tag(branch_cdb_tag), .i_rob_head(commit_tag)            
  );
  reservation_station #(.PREG_WIDTH(7), .ROB_WIDTH(4), .RS_SIZE(8)) rs_lsu_inst (
      ...
      .i_prd(skid_to_dispatch_prd), .i_rob_tag(skid_to_dispatch_rob_tag), 
      .i_imm(skid_to_dispatch_immediate), .i_alu_op(skid_to_dispatch_alu_op), 
      .i_alusrc(1'b1), .i_memwrite(skid_to_dispatch_memwrite), 
      .i_cdb_valid(cdb_valid), .i_cdb_prd(cdb_prd),
      .i_rs1_ready((skid_to_dispatch_prs1 == 0) || phys_reg_busy[skid_to_dispatch_prs1]),
      .i_rs2_ready((skid_to_dispatch_prs2 == 0) || phys_reg_busy[skid_to_dispatch_prs2]),
      .o_full(lsu_rs_full), .i_eu_ready(lsu_ready_to_rs), 
      ...
      .o_issue_memwrite(lsu_issue_memwrite), .branch_mispredict(branch_mispredict),
      .i_mispredict_tag(branch_cdb_tag), .i_rob_head(commit_tag)
  );
  ...
  physical_register_file #(.DATA_WIDTH(32), .PREG_WIDTH(7)) prf_inst (
      ...
      // NEW: Wire Branch Writeback
      .br_wb_valid(branch_wb_valid), .br_wb_dest(branch_wb_dest), .br_wb_data(branch_wb_data)
  );
  ...
  branch_unit #(.DATA_WIDTH(32), .ROB_WIDTH(4), .PREG_WIDTH(7)) branch_instance (
      ...
      // NEW: Output Link Data and PRD
      .o_data(branch_wb_data), .o_prd(branch_wb_dest),
  ...
  lsu_unit #(.DATA_WIDTH(32), .ROB_WIDTH(4), .PREG_WIDTH(7)) lsu_instance (
      ...
      .i_stall(lsu_stall), .i_alu_op(lsu_issue_op), .o_ready(lsu_ready_to_rs),
